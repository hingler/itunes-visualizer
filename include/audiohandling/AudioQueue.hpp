...
class AudioQueue {
 ...
 public:
   ...
   *  Truncates queue's contents if `newlen` < current queue size.
   *  Args:
   *    - newlen, the new length desired for the buffer.
   */ 
  void Resize(int newlen) {
    BUFFER_UNIT* newbuf = new BUFFER_UNIT[newlen];
    int32_t index;
    for (int i = 0; i < buffer_size_; i++) {
      index = (buffer_start_ + i) % buffer_capacity_;
      newbuf[i] = buffer_[index];
    }
